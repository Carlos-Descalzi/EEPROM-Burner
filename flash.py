...
def do_write(data,address,port):
	...
	while len(chunk) > 0:
		...
		ack = map(ord,port.read(2))

		if ack[0] == RESPONSE_OK:
			if crc != ack[1]:
				print 'Error in Receive CRC, local %d, writer:%d' % (crc,ack[1])
				port.write(chr(0xFF))
				return False
			else:
				port.write(chr(0x01))
		...
		if ack[0] == RESPONSE_OK:
			if crc != ack[1]:
				print 'Error in Write CRC, local %d, writer:%d' % (crc,ack[1])
				port.write(chr(0xFF))
				return False
			else:
				port.write(chr(0x01))
...
def write(port,baud,rom,address):
	...
	try:
		with open(rom,'rb') as f: data = f.read()
	...
	try:
		...
		print 'Unable to write to serial port: %s' % str(e)
		return 1
...
def read(port,baud,output,address,length):
	...
	try:
		...
		print 'Unable to read from serial port: %s' % str(e)
		return 1
	
	try:
		with open(output,'wb') as f: f.write(contents)
	except Exception as e:
		print 'Unable to open file: %s' % str(e)
		return 1
...
def readbyte(port,baud):
	...
	try:
		...
		print 'Unable to read serial port: %s' % str(e)
		return 1
